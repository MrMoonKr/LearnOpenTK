...
// One point light's diffuse + specular contribution, falling off with distance. Identical to
// 14-GpuPbr's hero shader (see this project's README for why the ground uses a separate, simpler
// shader instead of this one).
...
void main()
{
    ...
    // lights above don't reach are not pure black (same technique as 14-GpuPbr).
