...
// Same instancing scheme as 13-GpuSkinnedInstancing/14-GpuPbr: one bone palette and one placement
// matrix per instance, both indexed by gl_InstanceID. This project only ever draws 1 instance at a
// time (the selected hero), but reuses the same layout so the shader needs no changes from 14-GpuPbr.
