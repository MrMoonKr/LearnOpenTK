...
public sealed class MainForm : Form
{
    ...
    private readonly ToolStripMenuItem _animationsMenu = new("Animation");
    ...
    public MainForm()
    {
        ...
        help.DropDownItems.Add("About", null, (_, _) => MessageBox.Show(
            "Bakes bone indices and weights into each vertex once, then uploads only the bone matrix palette (uBones) every frame; the vertex shader performs the linear-blend skin instead of the CPU (compare 11-SkeletalAnimation).",
            "GPU Skinning"));
        menu.Items.AddRange([file, view, _animationsMenu, help]);
    ...
    private void RefreshScene(string statusText)
    {
        ...
        _animationsMenu.DropDownItems.Clear();
        foreach (var clipName in _view.AnimationNames)
        {
            _animationsMenu.DropDownItems.Add(clipName, null, (_, _) => _view.PlayClip(clipName));
        }
    }
